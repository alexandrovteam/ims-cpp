...
static_assert(sizeof(kiss_fft_scalar) == 8, "use -Dkiss_fft_scalar for double precision");
...
Spectrum computeIsotopePattern(
    const ms::Element& element, size_t amount, double threshold) {
  size_t dim = element.isotope_pattern.size() - 1;
  if (dim == 0) return ms::Spectrum(element.isotope_pattern.masses[0] * amount);
...
Spectrum computeIsotopePattern(
    const ElementCounter& element_counts, double threshold, double fft_threshold) {
  assert(threshold < 1);
  ms::Spectrum result(0);
  ...
  for (auto& item : element_counts) {
    ...
    try {
      ...
      result = result.convolve(pattern, threshold * threshold);
    } catch (std::runtime_error& e) {
      std::stringstream full_desc;
      full_desc << "failed to compute isotope pattern for " << item.first << item.second
                << ": " << e.what();
...
Spectrum computeIsotopePattern(
    const std::string& sum_formula, double threshold, double fft_threshold) {
