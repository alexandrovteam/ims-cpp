...
class Image {
 ...
 public:
  ...
  T& intensity(size_t x, size_t y) { return intensities_[pixelIndex(x, y, width_)]; }

  T* rawPtr() { return &intensities_[0]; }

  size_t countEmptyPixels() const {
    size_t n = 0;
    for (auto& x : intensities_)
      if (x < -0.5) ++n;
    return n;
  }
  ...
  std::pair<size_t, size_t> shape() const { return std::make_pair(height(), width()); }
  ...
  void removeHotspots(double percentile, T* tmpbuf = nullptr) {
    ...
    for (T i : intensities_)
      if (i > 0) *pend++ = i;
    if (tmpbuf == pend) return;  // empty image
