...
imzb::ImzbReader::ImzbReader(const std::string& filename)
  ...
  if (!in_.is_open()) throw std::runtime_error("couldn't open the file");
  if (!in_idx.is_open()) throw std::runtime_error("couldn't open the index file");
...
size_t imzb::ImzbReader::decompressBlock(size_t block_idx, std::ifstream& in,
    std::vector<char>& inbuf, std::vector<ims::Peak>& outbuf) const {
...
std::vector<ims::Peak> imzb::ImzbReader::slice(double min_mz, double max_mz) const {
  ...
  for (size_t i = start_block; i < end_block; ++i) {
    ...
    if (first_mz < min_mz)
      beg = std::lower_bound(
          beg, end, min_mz, [](const ims::Peak& p, double mz) { return p.mz < mz; });
    if (last_mz > max_mz)
      end = std::upper_bound(
          beg, end, max_mz, [](double mz, const ims::Peak& p) { return mz < p.mz; });
...
void imzb::ImzbReader::readImage(double mz, double ppm, float* image) const {
  ...
  for (auto& peak : peaks) {
    if (unlikely(!index_->header.mask.hasSpectrumAt(peak.coords.x, peak.coords.y))) {
      std::stringstream ss;
      ss << "peak at x=" << peak.coords.x << ", y=" << peak.coords.y
         << ", m/z=" << peak.mz << " is outside the spatial mask";
      throw std::runtime_error(ss.str());
    }

    if (unlikely(peak.intensity < 0)) {
      std::stringstream ss;
      ss << "negative intensity peak at x=" << peak.coords.x << ", y=" << peak.coords.y
         << ", m/z=" << peak.mz;
      throw std::runtime_error(ss.str());
    }
...
void imzb::ImzbReader::readCentroidedImage(double mz, double ppm, float* image) const {
  ...
  for (auto& peak : peaks) {
    ...
    if (i_prev[i] < i_curr[i] && i_curr[i] >= i_next[i]) { image[i] += i_curr[i]; }
  }
}
