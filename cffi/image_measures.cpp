...
extern "C" {
  ...
  IMS_EXTERN double iso_img_correlation_f(float** images, int n, int width, int height, void* isotope_pattern)
  {
    return datacube_score<float, isotopeImageCorrelation>(images, n, width, height, isotope_pattern);
  }

  IMS_EXTERN double iso_img_correlation_d(double** images, int n, int width, int height, void* isotope_pattern)
  {
    return datacube_score<double, isotopeImageCorrelation>(images, n, width, height, isotope_pattern);
  }

  IMS_EXTERN double pattern_match_f(float** images, int n, int width, int height, void* isotope_pattern)
  {
    return datacube_score<float, isotopePatternMatch>(images, n, width, height, isotope_pattern);
  }

  IMS_EXTERN double pattern_match_d(double** images, int n, int width, int height, void* isotope_pattern)
  {
