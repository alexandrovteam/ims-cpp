...
typedef double (*MultiF)(const std::vector<ims::ImageF>&, const std::vector<double>&);

static double isotopeImageCorrelation2(
...
double datacube_score(
    T** images, int n, int width, int height, double* isotope_abundances) {
...
IMS_EXTERN double iso_img_correlation_f(
    float** images, int n, int width, int height, double* abundances) {
  return datacube_score<float, isotopeImageCorrelation2>(
      images, n, width, height, abundances);
}

IMS_EXTERN double iso_img_correlation_d(
    double** images, int n, int width, int height, double* abundances) {
  return datacube_score<double, isotopeImageCorrelation2>(
      images, n, width, height, abundances);
}

IMS_EXTERN double pattern_match_f(
    float** images, int n, int width, int height, double* abundances) {
  return datacube_score<float, isotopePatternMatch>(images, n, width, height, abundances);
}

IMS_EXTERN double pattern_match_d(
    double** images, int n, int width, int height, double* abundances) {
  return datacube_score<double, isotopePatternMatch>(
      images, n, width, height, abundances);
}
}
